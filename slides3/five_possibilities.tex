...
\begin{frame}
    ...
    In the $\mathfrak{E}_1$ space, all three aspects converge:
    ...
    The space has a hyperbolic metric $ds^2 = \frac{1}{y^2}(dx^2 + dy^2)$.
    \end{block}
    ...
    This triad suggests that AEG may serve as a Rosetta Stone for translating among these domains.
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Purple line: $\left(1 - \frac{5}{8}\right) \times 8 = 3$
        \item Orange line: a specific integral curve
    \end{itemize}
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            With three steps, the pattern continues:
            \begin{equation}
...
\begin{frame}
    ...
    \end{itemize}
    Does this trade-off imply a common source of complexity? Which notion of complexity does geometric volume quantify?
\end{frame}
...
\begin{frame}
    \frametitle{Beyond Newton and Leibniz}
    Traditional calculus is fundamentally additive:
    \[
        \int_a^b f(x) dx = \lim_{n \to \infty} \sum_{i=1}^n f(x_i) \Delta x
    \]
    
    But what about mixed operations? We propose:
    
    \begin{block}{Neo-Integral}
    \[
        \mathcal{I}_\gamma[f] = \lim_{n \to \infty} \left( \prod_{i \in M} e^{f(s_i)\Delta s_i} \right) \left( \sum_{j \in A} g(s_j)\Delta s_j \right)
    \]
    where the path $\gamma$ determines the mixing pattern of multiplicative (M) and additive (A) steps.
    \end{block}
    
    This neo-calculus would:
    \begin{itemize}
        \item Unify discrete and continuous operations
        \item Naturally handle non-commutative processes
        \item Provide new tools for analyzing complex systems
    \end{itemize}
\end{frame}

\begin{frame}
    \frametitle{Flow Derivatives and Path Integrals}
    In this neo-calculus, we have new types of derivatives and integrals:
    
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \textbf{Flow Derivative}
            \[
                D_\theta f = \cos\theta \frac{\partial f}{\partial u} + \sin\theta \cdot f \frac{\partial \log f}{\partial v}
            \]
            
            This combines:
            \begin{itemize}
                \item Additive change (partial derivative)
                \item Multiplicative change (logarithmic derivative)
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Path Integral}
            \[
                \int_\gamma \omega = \int_0^L f(s) \cdot \text{mix}(\theta(s)) \, ds
            \]
            
            Where $\text{mix}(\theta)$ encodes the operation:
            \begin{itemize}
                \item $\theta = 0$: pure addition
                \item $\theta = \pi/2$: pure multiplication
                \item Other angles: mixed operations
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{figure}[ht]\centering
            \resizebox{0.9\textwidth}{!}{\includegraphics{../images/fractal}}
